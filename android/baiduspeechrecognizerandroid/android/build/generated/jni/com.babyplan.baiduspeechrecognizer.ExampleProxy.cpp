...
using namespace v8;
		...
		namespace baiduspeechrecognizer {
			namespace baiduspeechrecognizerandroid {
...
Handle<FunctionTemplate> ExampleProxy::getProxyTemplate()
{
	...
	javaClass = titanium::JNIUtil::findClass("com/babyplan/baiduspeechrecognizer/ExampleProxy");
	HandleScope scope;
...
, javaClass, nameSymbol);
	...
	// Method bindings --------------------------------------------------------
	DEFINE_PROTOTYPE_METHOD(proxyTemplate, "start", ExampleProxy::start);
	DEFINE_PROTOTYPE_METHOD(proxyTemplate, "stop", ExampleProxy::stop);
	DEFINE_PROTOTYPE_METHOD(proxyTemplate, "cancel", ExampleProxy::cancel);
...
// Methods --------------------------------------------------------------------
Handle<Value> ExampleProxy::start(const Arguments& args)
{
	LOGD(TAG, "start()");
	HandleScope scope;
	...
	if (!methodID) {
		methodID = env->GetMethodID(ExampleProxy::javaClass, "start", "(Lorg/appcelerator/kroll/KrollFunction;)V");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'start' with signature '(Lorg/appcelerator/kroll/KrollFunction;)V'";
			LOGE(TAG, error);
...
Handle<Value> ExampleProxy::stop(const Arguments& args)
{
	LOGD(TAG, "stop()");
	HandleScope scope;
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ExampleProxy::javaClass, "stop", "()V");
		if (!methodID) {
...
Handle<Value> ExampleProxy::cancel(const Arguments& args)
{
	LOGD(TAG, "cancel()");
	HandleScope scope;
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ExampleProxy::javaClass, "cancel", "()V");
		if (!methodID) {
